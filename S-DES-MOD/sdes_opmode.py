...
def sdes_encrypt_ecb(text: bitarray, key: bitarray):
    # text를 8비트씩 나눠서 sdes로 암호화 - 어떻게 합칠것인지?
    ciphertext = bitarray()
    for i in range(len(text)//8):
        cipher_block = sdes(text[i*8:(i+1)*8], key, MODE_ENCRYPT)
        ciphertext += cipher_block
    return ciphertext

def sdes_decrypt_ecb(ciphertext: bitarray, key: bitarray):
    # ciphertext를 8비트씩 나눠서 sdes로 복호화
    text = bitarray()
    
    for i in range(len(ciphertext)//8):
        plain_block = sdes(ciphertext[i*8:(i+1)*8], key, MODE_DECRYPT)
        text += plain_block
    return text

def sdes_encrypt_cbc(text: bitarray, key: bitarray, iv:bitarray):
    # text를 8비트씩 나눠서 sdes로 암호화
    ciphertext = bitarray()

    x = iv
    for i in range(len(text)//8):
        block = x ^ text[i*8:(i+1)*8]
        block = sdes(block, key, MODE_ENCRYPT)
        x = block
        ciphertext += block
...
def sdes_decrypt_cbc(ciphertext: bitarray, key: bitarray, iv:bitarray):
    ...
    for i in range(len(ciphertext)//8):
        ...
        plain_block = sdes(block, key, MODE_DECRYPT)
        plain_block ^= x
        x = block
        text += plain_block
        print(text)
