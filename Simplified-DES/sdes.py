...
def schedule_keys(key: bitarray) -> list[bitarray]:
    ...
    # round_keys[0] = k1 , round_keys[1] = k2
    return round_keys
...
def sdes(text: bitarray, key: bitarray, mode) -> bitarray:
    result = bitarray()
    k1, k2 = schedule_keys(key)
    # Place your own implementation of S-DES Here
    # initial permutation
    # expansion permutation - ip의 오른쪽 4비트 값을 입력하여 8비트 값으로 확장
    # 확장된 값을 k1과 xor
    # 그걸 좌우 4비트씩 나누어서 각각 s0, s1 투입 -> 각각 2비트씩 나옴 -> p4
    # ep단계 left 4비트와 p4 xor
    # 그 결과와 ip right 4bit를 붙임 -> switch 함수에 입력
    #--------------round 1 ----------------------
    if mode == 1:
        initial_permutate_text = bitarray()
        for i in IP:
            initial_permutate_text.append(text[i])
        round_result = round(initial_permutate_text[4:], k1)
        round_result ^= initial_permutate_text[0:4]
        r1_result = initial_permutate_text[4:] + round_result
        
        round_result_2 = round(r1_result[4:], k2)
        round_result_2 ^= r1_result[0:4]
        r2_result = round_result_2 + r1_result[4:]
        for i in IP_1:
            result.append(r2_result[i])
        
    if mode == 2:
        initial_permutate_text = bitarray()
        for i in IP:
            initial_permutate_text.append(text[i])
        round_result = round(initial_permutate_text[4:], k2)
        round_result ^= initial_permutate_text[0:4]
        r1_result = initial_permutate_text[4:] + round_result
        
        round_result_2 = round(r1_result[4:], k1)
        round_result_2 ^= r1_result[0:4]
        r2_result = round_result_2 + r1_result[4:]
        for i in IP_1:
            result.append(r2_result[i])
